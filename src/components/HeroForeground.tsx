...
const MID_SPECS: CardSpec[] = [
  {
    ...
    buildTl: (el, W, H) =>
      gsap.timeline({ repeat: -1, delay: 0 })
        ...
        .to(el,   { x: W * 0.22,  y: H * 0.23, rotation:  8, scale: 1.02, duration: 17, ease: 'sine.inOut' })
        .to(el,   { x: W * 0.58,  y: H * 0.46, rotation: -6, scale: 0.96, duration: 19, ease: 'sine.inOut' })
        .to(el,   { x: W + 220,   y: H * 0.36, rotation: 10, scale: 1.00, duration: 16, ease: 'sine.inOut' }),
  },
  {
...
const FRONT_SPECS: CardSpec[] = [
  {
    ...
    buildTl: (el, W, H) =>
      gsap.timeline({ repeat: -1, delay: 4 })
        ...
        .to(el,   { x: W * 0.66, y: H * 0.14, rotation: -7, scale: 1.04, duration: 18, ease: 'sine.inOut' })
        .to(el,   { x: W * 0.26, y: H * 0.40, rotation:  9, scale: 0.96, duration: 22, ease: 'sine.inOut' })
        .to(el,   { x: -250,     y: H * 0.58, rotation: -5, scale: 1.00, duration: 17, ease: 'sine.inOut' }),
  },
  {
    ...
    buildTl: (el, W, H) =>
      gsap.timeline({ repeat: -1, delay: 13 })
        .set(el,  { x: -200,     y: H * 0.74, rotation:   0, scale: 1.00 })
        .to(el,   { x: W * 0.18, y: H * 0.50, rotation: -11, scale: 1.02, duration: 22, ease: 'sine.inOut' })
        .to(el,   { x: W * 0.50, y: H * 0.26, rotation:   7, scale: 0.95, duration: 24, ease: 'sine.inOut' })
        .to(el,   { x: W * 0.80, y: -240,     rotation:  -5, scale: 1.08, duration: 18, ease: 'sine.inOut' }),
  },
  {
